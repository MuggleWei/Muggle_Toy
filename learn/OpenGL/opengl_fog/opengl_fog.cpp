...
static GLuint fog_types[GTFT_MAX] = { 0 };
static GLuint fog_index = 0;
...
void Render()
{
	...
	shader_program.setUniform("Fog.minDist", 10.0f);
	shader_program.setUniform("Fog.maxDist", 25.0f);
	shader_program.setUniform("Fog.density", 0.05f);
	shader_program.setUniform("Fog.color", muggle::vec3f(0.3f, 0.3f, 0.3f));
	...
	for (int i = 0; i < 5; ++i)
	{
		mat_model = muggle::MathUtils::Translate(muggle::vec3f(1.0f * i, 0.0f, 2.0f * i));
		...
		glUniformSubroutinesuiv(GL_FRAGMENT_SHADER, 1, &fog_index);
...
void PrepareShader()
{
	const char* vert_shader_name = "res_learn_opengl/shaders/fog_vert.glsl";
	const char* frag_shader_name = "res_learn_opengl/shaders/fog_frag.glsl";
	...
	fog_types[GTFT_Linear] = glGetSubroutineIndex(shader_program.getHandle(), GL_FRAGMENT_SHADER, "linearFog");
	fog_types[GTFT_Exponential] = glGetSubroutineIndex(shader_program.getHandle(), GL_FRAGMENT_SHADER, "exponentialFog");
	for (int i = 0; i < GTFT_MAX; ++i)
	{
		if (GL_INVALID_INDEX == fog_types[i])
		{
			MASSERT(0);
		}
	}
